...
export const loader = async ({ params }) => {
  ...
  const event = get(child(dbRef, `events/${params.eventId}`))
    .then((snapshot) => {
      if (snapshot.exists()) {
  ...
  // await fetch(
  //   `https://eventapp-bbafa-default-rtdb.europe-west1.firebasedatabase.app/events/${params.eventId}.json`
  // );
  ...
  //  await fetch(
  //   "https://eventapp-bbafa-default-rtdb.europe-west1.firebasedatabase.app/users.json"
  // );

  const categories = get(child(dbRef, `categories`))
    .then((snapshot) => {
      if (snapshot.exists()) {
        console.log(snapshot.val());
      } else {
  ...
  // await fetch(
  //   "https://eventapp-bbafa-default-rtdb.europe-west1.firebasedatabase.app/categories.json"
  // );
  return {
...
export const EventPage = () => {
  ...
  const deleteEvent = async (eventId) => {
    const dbRef = ref(getDatabase());
    remove(child(dbRef, `events/${eventId}`))
      .then((snapshot) => {
        if (snapshot.exists()) {
          console.log(snapshot.val());
        } else {
  ...
  //   try {
  //     await fetch(
  //       `https://eventapp-bbafa-default-rtdb.europe-west1.firebasedatabase.app/events/${eventId}.json`,
  //       {
  //         method: "DELETE",
  //       }
  //     );

...
export const loader = async () => {
  ...
  const events = get(child(dbRef, `events/events?_page=1&_limit=5`))
    .then((snapshot) => {
      if (snapshot.exists()) {
  ...
  // await fetch(
  //   `https://eventapp-bbafa-default-rtdb.europe-west1.firebasedatabase.app/events/events?_page=1&_limit=5.json`
  // );

  const categories = ref(getDatabase());
...
export const EventsPage = () => {
  ...
  const moreEventsLoader = async () => {
    const moreEvents = ref(getDatabase());
    get(child(moreEvents, `events/events?_page=1&_limit=200`))
      .then((snapshot) => {
        if (snapshot.exists()) {
          console.log(snapshot.val());
        } else {
